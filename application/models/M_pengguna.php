...
class M_pengguna extends CI_Model {
	...
	function get_all_pengguna() {
		$hsl = $this->db->query("SELECT tbl_user.*,IF(jenkel='L','Laki-Laki','Perempuan') AS jenkel FROM tbl_user");
		return $hsl;
	}
	...
	function getusername($id) {
		$hsl = $this->db->query("SELECT * FROM tbl_user where id='$id'");
		return $hsl;
	}
	function resetpass($id, $pass) {
		$hsl = $this->db->query("UPDATE tbl_user set password=md5('$pass') where id='$id'");
		return $hsl;
	}

	function get_pengguna_login($kode) {
		$hsl = $this->db->query("SELECT * FROM tbl_user where id='$kode'");
		return $hsl;
	}
