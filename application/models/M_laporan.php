...
class M_laporan extends CI_Model{
	...
	function get_data_penjualan(){
		$hsl=$this->db->query("SELECT jual_nofak,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,jual_total,d_jual_barang_id,d_jual_barang_nama,d_jual_barang_satuan,d_jual_barang_harpok,d_jual_barang_harjul,d_jual_qty,d_jual_diskon,d_jual_total FROM tbl_jual JOIN tbl_detail_jual ON jual_nofak=d_jual_nofak  WHERE kode = 'BR' ORDER BY jual_nofak DESC");
		return $hsl;
	}
	function get_total_penjualan(){
		$hsl=$this->db->query("SELECT jual_nofak,DATE_FORMAT(jual_tanggal,'%d %M %Y') AS jual_tanggal,jual_total,d_jual_barang_id,d_jual_barang_nama,d_jual_barang_satuan,d_jual_barang_harpok,d_jual_barang_harjul,d_jual_qty,d_jual_diskon,sum(d_jual_total) as total FROM tbl_jual JOIN tbl_detail_jual ON jual_nofak=d_jual_nofak WHERE kode = 'BR' ORDER BY jual_nofak DESC");
		return $hsl;
	}
	...
	function get_data_beli_pertanggal($tanggal){
		$hsl=$this->db->query("SELECT beli_nofak,DATE_FORMAT(beli_tanggal,'%d %M %Y') AS beli_tanggal,beli_suplier_id,d_beli_id,d_beli_barang_id,d_beli_harga,d_beli_jumlah,d_beli_total FROM tbl_beli JOIN tbl_detail_beli ON beli_nofak=d_beli_nofak WHERE DATE(beli_tanggal)='$tanggal' ORDER BY beli_nofak DESC");
		return $hsl;
	}
	function get_data__total_beli_pertanggal($tanggal){
		$hsl=$this->db->query("SELECT beli_nofak,DATE_FORMAT(beli_tanggal,'%d %M %Y') AS beli_tanggal,beli_suplier_id,d_beli_id,d_beli_barang_id,d_beli_harga,d_beli_jumlah,sum(d_beli_total) as total FROM tbl_beli JOIN tbl_detail_beli ON beli_nofak=d_beli_nofak WHERE DATE(beli_tanggal)='$tanggal' ORDER BY beli_nofak DESC");
		return $hsl;
	}
	...
	function get_beli_pertahun($tahun){
		$hsl=$this->db->query("SELECT beli_nofak,YEAR(beli_tanggal) AS tahun,DATE_FORMAT(beli_tanggal,'%M %Y') AS bulan,DATE_FORMAT(beli_tanggal,'%d %M %Y') AS beli_tanggal,beli_suplier_id,d_beli_id,d_beli_barang_id,d_beli_harga,d_beli_jumlah,d_beli_total FROM tbl_beli JOIN tbl_detail_beli ON beli_nofak=d_beli_nofak WHERE YEAR(beli_tanggal)='$tahun' ORDER BY beli_nofak DESC");
		return $hsl;
	}
	function get_total_beli_pertahun($tahun){
		$hsl=$this->db->query("SELECT beli_nofak,YEAR(beli_tanggal) AS tahun,DATE_FORMAT(beli_tanggal,'%M %Y') AS bulan,DATE_FORMAT(beli_tanggal,'%d %M %Y') AS beli_tanggal,beli_suplier_id,d_beli_id,d_beli_barang_id,d_beli_harga,d_beli_jumlah,SUM(d_beli_total) as total  FROM tbl_beli JOIN tbl_detail_beli ON beli_nofak=d_beli_nofak WHERE YEAR(beli_tanggal)='$tahun' ORDER BY beli_nofak DESC");
		return $hsl;
	}
	//=========Laporan Laba rugi============
