...
class Album extends CI_Controller {
	...
	function index() {
		if($this->session->userdata('akses')=='1' ||$this->session->userdata('akses')=='2'){
			...
			$this->load->view('admin_web/v_album', $x);
		}else{
	...
	function simpan_album() {
			...
			if (!empty($_FILES['filefoto']['name'])) {
				if ($this->upload->do_upload('filefoto')) {
					...
					$p = $user->row_array();
					$user_id = $p['id'];
					$this->m_album->simpan_album($album, $user_id, $gambar);
					echo $this->session->set_flashdata('msg', 'success');
					redirect('admin_web/album');
				} else {
	...
	function update_album() {
			...
			if (!empty($_FILES['filefoto']['name'])) {
				if ($this->upload->do_upload('filefoto')) {
					...
					$p = $user->row_array();
					$user_id = $p['id'];
					$this->m_album->update_album($album_id, $album_nama, $user_id, $gambar);
				...
				$p = $user->row_array();
				$user_id = $p['id'];
				$this->m_album->update_album_tanpa_img($album_id, $album_nama, $user_id);
				echo $this->session->set_flashdata('msg', 'info');
				redirect('admin_web/album');
