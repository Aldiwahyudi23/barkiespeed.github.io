...
class M_album extends CI_Model {
	function get_all_album() {
		$hsl = $this->db->query("SELECT tb_album.id, tb_album.nama,tb_album.count, tb_album.cover, tbl_user.nama as author,DATE_FORMAT(tb_album.tanggal,'%d/%m/%Y') AS tanggal FROM tb_album inner join tbl_user on tb_album.userid = tbl_user.id ORDER BY tb_album.id DESC");
		return $hsl;
	}
