...
$b = $data->row_array();
?>
        <form action="<?php echo base_url() . 'admin_web/visimisi/update_visimisi' ?>" method="post" enctype="multipart/form-data">
...
<?php if ($this->session->flashdata('msg') == 'info'): ?>
  ...
  $.toast({
    heading: 'Info',
    text: "visimisi berhasil di update",
...
<?php elseif ($this->session->flashdata('msg') == 'info1'): ?>
 ...
 $.toast({
  heading: 'Info',
  text: "visimisi berhasil di update tdengan gambar tetap",
  showHideTransition: 'slide',
  icon: 'info',
  hideAfter: false,
  position: 'bottom-right',
