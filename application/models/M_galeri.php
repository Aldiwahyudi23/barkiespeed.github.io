...
class M_galeri extends CI_Model {
	...
	function get_all_galeri() {
		$hsl = $this->db->query("SELECT tb_gallery.*,DATE_FORMAT(tb_gallery.tanggal,'%d/%m/%Y') AS tanggal, tb_album.nama as album, tbl_user.nama as author FROM tb_gallery join tb_album on albumid=tb_album.id inner join tbl_user on tb_gallery.userid = tbl_user.id ORDER BY tb_gallery.id DESC");
		return $hsl;
	}
	...
	function get_galeri_home() {
		$hsl = $this->db->query("SELECT tb_gallery.*,DATE_FORMAT(tb_gallery.tanggal,'%d/%m/%Y') AS tanggal, tb_album.nama as album, tbl_user.nama as author FROM tb_gallery join tb_album on albumid=tb_album.id inner join tbl_user on tb_gallery.userid = tbl_user.id ORDER BY tb_gallery.id DESC limit 6");
		return $hsl;
	}

	function get_galeri_by_album_id($idalbum) {
		$hsl = $this->db->query("SELECT tb_gallery.*,DATE_FORMAT(tb_gallery.tanggal,'%d/%m/%Y') AS tanggal, tb_album.nama as album, tbl_user.nama as author FROM tb_gallery join tb_album on albumid=tb_album.id inner join tbl_user on tb_gallery.userid = tbl_user.id where albumid = '$idalbum'ORDER BY tb_gallery.id DESC");
		return $hsl;
	}
