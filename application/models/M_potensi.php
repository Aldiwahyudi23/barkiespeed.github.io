...
class M_potensi extends CI_Model {
	...
	function tampil_potensi() {
		$hsl = $this->db->query("SELECT tb_potensi.id, tb_potensi.judul, tb_potensi.tanggal, tb_potensi.isi, tb_potensi.slug,tb_potensi.gambar,tbl_user.nama AS author, DATE_FORMAT(tanggal,'%d/%m/%Y') AS tanggal1 FROM tb_potensi INNER join tbl_user on tb_potensi.userid = tbl_user.id ORDER BY tb_potensi.tanggal DESC");
		return $hsl;
	}
	...
	function get_potensid_by_kode($kode) {
		$hsl = $this->db->query("SELECT tb_potensi.id, tb_potensi.judul, tb_potensi.tanggal, tb_potensi.isi, tb_potensi.slug,tb_potensi.gambar,tbl_user.nama AS author, DATE_FORMAT(tanggal,'%d %M %Y %h:%i') AS tanggal1 FROM tb_potensi INNER join tbl_user on tb_potensi.userid = tbl_user.id where tb_potensi.id='$kode'");
		return $hsl;
	}
